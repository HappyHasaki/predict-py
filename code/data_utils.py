...
def fill_stock_data(target, sfile, tfile):
	...
	for item in csv_list:
		...
		d1.rename(columns={'open_y': 'open', 'high_y': 'high', 'close_y': 'close', 'low_y': 'low',  'volume_y': 'volume', 'p_change_y': 'p_change'}, inplace=True)
...
def get_bank_stock_data():
	pro = ts.pro_api('1bf2b34c8f1b11ea031f7b95d12ca7e9f04142fa7e4e1e9353fff854')
	bank_list = pro.stock_basic(exchange='', list_status='L', fields='ts_code,symbol,name,area,industry,list_date')
	bank_list = bank_list[['ts_code','industry']]
	df=pd.DataFrame()
	num=0
	for index,row in bank_list.iterrows():
		if(row['industry']=='银行'):
			num+=1
			code = row['ts_code'][:6]
			print(code)
			df1 = ts.get_hist_data(str(code), start='2021-01-01', end='2021-04-13')
			df1=df1.sort_values(by='date')
			df1 = df1[['open', 'high', 'close', 'low', 'volume', 'p_change']]
			df = pd.concat([df, df1])
			print(len(df1))
	df.to_csv('d:\WorkSpace\\git\\hs300_stock_predict\\data\\bank_test_mix.csv')
	print(num)

def get_stock_list():
	pro = ts.pro_api('1bf2b34c8f1b11ea031f7b95d12ca7e9f04142fa7e4e1e9353fff854')
	stock_list = pro.stock_basic(exchange='', list_status='L', fields='ts_code,symbol,name,area,industry,list_date')
	stock_list=stock_list[['ts_code','symbol','name','area','industry','list_date']]
	# for index,row in stock_list.iterrows():
	# 	row['ts_code']=row['ts_code'][:6]
	type = 'list.csv'
	stock_list.to_csv(os.path.join('d:\data\\', type),encoding='gbk')

def addToMysql(code_list, predict_list):
    # 连接数据库
    host = 'localhost'
    port = 3306
    user = 'root'
    password = 'ershou666@A.'
    db = 'predict'
    ...
    # sql语句
    sql = "insert into prediction (id, code,predict_price,create_time) values (%s,%s,%s,%s)"
    # 获取游标
    cur = conn.cursor()
    # print(len(title_list))
    for i in range(len(code_list)):
        # 参数化方式传参
        row_count = cur.execute(sql, [0, code_list[i], predict_list[i],
...
if __name__ == '__main__':
	get_bank_stock_data()
	# print(1)
	# get_hs300_data('2019/5/1','2019/5/31','d:\data\\')
	# fill_stock_data('d:\data\\399300.csv', 'd:\data\\', 'd:\data\\')
	# merge_stock_data('d:\data\\', 'd:\data\\')
