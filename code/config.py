...
class Arg:
    def __init__(self):
        # 所有上市股票代码
        self.stock_list_dir = '/data/list.csv'
        # 训练集数据存放路径
        self.train_dir = '/data/bank_train_mix.csv'
        # 测试集数据存放路径
        self.test_dir = '/data/bank_test_mix.csv'
        # 更新数据存放路径
        ...
        # fining-turn模型存放路径
        self.fining_turn_model_dir = '/model/new_logfile/finet'
        # 训练图存放路径
        self.train_graph_dir = '/model/new_logfile/graph/train_270\\'
        # 验证loss存放路径
        self.val_graph_dir = '/model/new_logfile/graph/val_270\\'
        # 模型名称
        self.model_name = 'model-270-17-19'
        self.model_name_ft = 'model-ft-01-03'
        self.rnn_unit = 32     # 隐层节点数
        self.input_size = 7     # 输入维度（既用几个特征）
        ...
        self.stock_len = 16689
        self.test_stock_len = 2484
        # 更新后单只股票的长度（同一数据集股票长度应处理等长）
        #self.stock_len_new = get_data_len('D:\data\\399300_190103.csv')
        self.stock_len_new = 1540
        self.test_stock_len_new = 2101
